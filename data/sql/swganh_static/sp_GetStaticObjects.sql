...
DELIMITER //
CREATE DEFINER=`swganh`@`localhost` PROCEDURE `sp_GetStaticObjects`(IN `parent_id` BIGINT, IN `planet_id` INT)
BEGIN
	...
	SELECT buildings.id,buildings.oX,buildings.oY,buildings.oZ,buildings.oW,buildings.x,
		...
		building_types.file,building_types.name, building_families.family_name
		FROM buildings INNER JOIN building_types ON buildings.type_id = building_types.id
		INNER JOIN building_families ON building_types.family_id = building_families.id
